...
if (!msg.equals("pen-up") && mouse_b) {
  ...
  Pointlist.add(new PVector(xFakeMouse, yFakeMouse));
...
public void endStroke() {
 if (tst != null && tst.getVertexCount() > 0) { //check for if tst is null
  ...
  for (int i=1;i<tst.getVertexCount();i=i+1){ //this is the part that adds the lines to the world
    ...
    /*
  for (int i=1;i<tst.getVertexCount()-1;i=i+1){ //this is the part that adds the lines to the world
    v1=(tst.getVertex(i-1));
    v2=(tst.getVertex(i));
